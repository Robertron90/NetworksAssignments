...
void client_request_file(int sockfd, char *file)
{
...

  {
    ...
    {
      sent_recv_bytes = recvfrom(sockfd, (char *)&result3, sizeof(result3) - 1, MSG_WAITALL, NULL, 0);
      printf("\n\rclient got: %s\n", result3);
...
void server_send_file(int comm_socket_fd)
{
  ...
  for (i = 0; i < words_count; i++)
  {
    ...
    sent_recv_bytes = sendto(comm_socket_fd, (char *)&current_word, sizeof(current_word) + 1, 0, NULL, 0);
    printf("\r\nserver -----------> client: %s\n", current_word);
  }
}
